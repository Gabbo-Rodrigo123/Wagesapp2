...
namespace WagesApp
{
    public class Program
    {
        ...
        List<string> questions = new List<string> { "Enter Employee ID", "Enter the first name\n", "Enter the last name\n" };
        ...
        static string OneEmployee()
        {
            ...
            //ID
            id = CheckInt("", IDMINMAX[0], IDMINMAX[5000]);

            //Name
            name = CheckName("");
            lastName = CheckName("Enter the Last name\n");

            //Hours worked
            foreach (string day in week)
            {
                Console.WriteLine($"Enter hours worked on {day}:");
                hoursWorked.Add(CheckInt($"Enter hours worked on {day}:", HOURSMINMAX[0], HOURSMINMAX[24]));
